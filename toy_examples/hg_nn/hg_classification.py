...
from toy_examples.flow_sphere.example_HGs import tetrahedron, OH_rectangle, random_S2_OH, rectangle, irregular_graph
from OrientedHypergraphs.train import update, evaluate, evaluate_F1, TrainingState
from morphomatics.graph.operators import max_pooling, mean_pooling
from morphomatics.nn import MfdInvariant, MfdFC
from morphomatics.nn import flow_layer as FlowLayer
from morphomatics.manifold import Manifold
import random
from OrientedHypergraphs.utils import pickle_load, pickle_dump
from OrientedHypergraphs.objects import OrientedHypergraph, OHtuple_to_jraph_edgeless, jraph_to_OH, OHGraphTupleReduced
...
def parameter_dashboard(t_hist, invariant_MfdFC_hist, invariant_linear_hist,
                        train_accuracies, test_accuracies, train_accuracies_acc, test_accuracies_acc, l,
                        save_fig_as=False):
    """
    ...
    fig.suptitle(f'{NUM_CLASSES}-Classes Generating-Algorithm Classification', fontsize='large')
...
class GraphClassifier(nn.Module):
    ...
    def __call__(self, OH: OHGraphTupleReduced, train: bool = True) -> jnp.ndarray:
        ...
        OH = OH._replace(X=jnp.concatenate([z[:, None, :], ] * self.width, axis=1))
...
def main(num_node: int, graph_num: int, n_train: int, n_epochs: int, M: Manifold, simple_network: bool = True,
         step_type: str = "explicit_flow", batch_padding: bool = False, batch_size: int = 1, learningrate: float = 1e-3,
         load_device: str = "gpu", save_fig_as: str = False, save_data_file_name: str = False, load_data_file_name: str = False):
    """
    ...
    if load_data_file_name:
        ...
        for i in range(graph_num):
            OH_1 = jraph_to_OH(tetrahedron(homogeneous_edge_weights=True)).OHGraphTupleReduced._replace(
                globals=jnp.array([0]))
            OH_2 = jraph_to_OH(irregular_graph(homogeneous_edge_weights=True)).OHGraphTupleReduced._replace(
                globals=jnp.array([1]))
            OH_3 = jraph_to_OH(irregular_graph(homogeneous_edge_weights=False)).OHGraphTupleReduced._replace(
                globals=jnp.array([2]))
            ...
            OH_1 = random_S2_OH(num_node, num_node, scale=0.01).OHGraphTupleReduced._replace(
                globals=jnp.array([0]))
            OH_2 = random_S2_OH(num_node, num_node, scale=0.1).OHGraphTupleReduced._replace(
                globals=jnp.array([1]))
            OH_3 = random_S2_OH(num_node, num_node, scale=1.0).OHGraphTupleReduced._replace(
                globals=jnp.array([2]))
            data.append(OH_1)
            data.append(OH_2)
            data.append(OH_3)
    ...
    print(f"Parameters of network are: \n WIDTH = {WIDTH}"
    ...
    for epoch in range(n_epochs):
        ...
        for step in range(n_train):
            ...
            state = update(state, train_dataset[step], train_dataset[step].globals, optimizer, model, key, mask)
            if epoch == 0 and step == 0:
                print("First state updated")
            l = int(n_train / 2)  # Calculate scores over full training and testing sets twice per epoch
            if (step + l) % (l + 1) == l:
                ...
                for g in train_dataset:
                    batch_accuracy_F1 = np.array(evaluate_F1(state.avg_params, g, g.globals, NUM_CLASSES, model,
                                                             key, mask=mask))
                    batch_accuracy_acc = np.array(evaluate(state.avg_params, g, g.globals, NUM_CLASSES, model,
                                                           key, mask=mask))
                ...
                print({"epoch.step": f"{epoch}.{step}", "train_average_F1": f"{train_accuracy_F1[0]:.3f}",
                       "test_average_F1": f"{test_accuracy_F1[0]:.3f}"})
                print({"epoch.step": f"{epoch}.{step}", "train_average_acc": f"{train_accuracy_acc[0]:.3f}",
                       "test_average_acc": f"{test_accuracy_acc[0]:.3f}"})

                t_hist.append(np.array(state.params['flow_layer']['t_sqrt'] ** 2))
                invariant_MfdFC_hist.append(np.array(state.params['MfdInvariant/~/MfdFC']['w']))
                invariant_linear_hist.append(np.array(state.params['MfdInvariant/~/linear']['w']))

                if evaluate_counter % int(n_epochs / 2) == 1:
                    parameter_dashboard(t_hist, invariant_MfdFC_hist, invariant_linear_hist,
                                        train_accuracies=train_accuracies_F1, test_accuracies=test_accuracies_F1,
                                        train_accuracies_acc=train_accuracies_acc,
                                        test_accuracies_acc=test_accuracies_acc, l=l)
                evaluate_counter += 1
    parameter_dashboard(t_hist, invariant_MfdFC_hist, invariant_linear_hist,
                        train_accuracies=train_accuracies_F1, test_accuracies=test_accuracies_F1,
                        train_accuracies_acc=train_accuracies_acc,
    ...
    print(f"Last 10 steps of F1: (training, testing) = ({jnp.array(train_accuracies_F1)[-10:].sum() / 10:.3f}, "
          f"{jnp.array(test_accuracies_F1)[-10:].sum() / 10:.3f}).")
    print(f"Last 10 steps of acc: (training, testing) = ({jnp.array(train_accuracies_acc)[-10:].sum() / 10:.3f}, "
