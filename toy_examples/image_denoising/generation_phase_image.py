...
# from toy_examples.example_HGs import OH_rectangle, tetrahedron
    ...
    for j in range(len(values)):
        for i in range(len(values)):
            ...
            for m in range(1, 9):
                ...
                in_range = abs(Xr + midP[0] + m * linewidth) + abs(Yr + midP[1] + m * linewidth)
                if in_range <= linewidth:
                    k3 -= 2 * np.pi * (m / 8)
                else:
                    k3 += 0.0
            ...
            if k1 + k2 + k3 - np.pi < -np.pi:
                pass
            img_noisy[j, i] = (
                np.mod(k1 + k2 + k3 - np.pi + np.random.normal(0, sigma), 2 * np.pi)
                + np.pi
            )
...
def graphify_image_non_local(
    img, k_nearest=5, patch_size=1, connect_middle=False, no_overlap=True
):
    """
    ...
    def matrix_to_stack_index(indexes, l_x=l_x, l_y=l_y):
        assert (
            indexes.shape[-1] == 1 or indexes.shape[-1] == 2
        ), "indexes should have shape where last axis is 1 or 2"
        ...
        for j in range(l_y):
            ...
            for i_comp in range(l_x):
                for j_comp in range(l_y):
                    if no_overlap:
                        if (
                            abs(i_comp - i) <= 2 * patch_size
                            and abs(j_comp - j) <= 2 * patch_size
                        ):
                            D_distances[i_comp, j_comp] = np.infty
                        else:
                            D_distances[i_comp, j_comp] = np.linalg.norm(
                                S1_distance(D[i, j], D[i_comp, j_comp])
                            )
            ...
            k_nearest_i = jnp.argpartition(D_distances.flatten(), kth=k_nearest)[
                0:k_nearest
            ]
            ...
            k_nearest_patches = D_ind_flat[k_nearest_ij[:, 0], k_nearest_ij[:, 1]][
                :, :, 0
            ]
            for k, D_k in enumerate(k_nearest_patches):
                ...
                w_scaling = (
                    jnp.sqrt((k_nearest - k) / k_nearest)
                    * D_distances.flatten()[k_nearest_i[0]]
                )
                if w_scaling == 0:
                    w_scaling = 1
                    print("w_scaling off")
                ...
                W.append(
                    [w_scaling / D_distances.flatten()[k_nearest_i[k]]]
                )  # Attempt also linear interpolation
            if (i * l_x + j) % (l_x * l_y // 10) == 0:
                print(
                    f"{i * l_x + j} of {l_x * l_y} pixels found k={k_nearest} nearest of"
                )
...
def phase_image_OH(
    ...
    if non_local:
        e2v_in, e2v_out, W = graphify_image_non_local(
            img_noisy,
            k_nearest=k_nearest,
        ...
        e2v_in, e2v_out = graphify_image(
            img_noisy,
            hyperedges=hyper_local,
            ForwardOH=ForwardOH,
            BackwardOH=BackwardOH,
        )
        W = None
    # e2v_in, e2v_out = jnp.array(e2v_in), jnp.array(e2v_out)
    img_noisy_cartesian = jnp.moveaxis(
        jnp.array([np.cos(img_noisy), np.sin(img_noisy)]), 0, -1
    )
