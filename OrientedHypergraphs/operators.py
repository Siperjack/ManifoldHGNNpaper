...
import jax
import numpy as np
from OrientedHypergraphs.objects import (OrientedHypergraph, OHGraphTupleReduced, calc_e2v2v_bool, OHtuple_to_jraph_edgeless,
                           OHGraphTupleReduced, flatten_list_of_arrays)
...
from morphomatics.graph.operators import vec_times_kd, mfdg_laplace
from jax import tree_util as tree
import jraph


def grad_F_unweighted(OH: OHGraphTupleReduced, Man: Manifold) -> tuple[jnp.array, jnp.array]:
    """
    ...
    for i in range(OH.num_edge):  # TODO: Make e_in_deg constant. One way is through introducing weights in Fréchet
...
def div_F_sym_weighted_squared(OH: OHGraphTupleReduced, H_f: jnp.array, base_points, Man: Manifold,
                               edge_normalize=False, deg_normalize=False):
    """
    ...
    PTedH_f = jax.vmap(Man.connec.transp)(base_points[PT_start], OH.X[PT_stop], H_f_weighted[PT_start])
    div_unormalized = jax.ops.segment_sum(PTedH_f, PT_stop, num_segments=OH.num_node)
    div = div_unormalized / D_in[..., None]
    return div


def FLaplace(OH: OHGraphTupleReduced, Man: Manifold,
             edge_normalize=False, deg_normalize=False):
    """
    ...
    Laplace = -div_F_sym_weighted_squared(OH, gradsF, base_points, Man,
                                          edge_normalize=edge_normalize, deg_normalize=deg_normalize)
    return Laplace
...
def grad_P_unweighted(OH: OHGraphTupleReduced, Man: Manifold) -> jnp.array:
    ...
    """
    indexes_logs_edges_out, indexes_logs_nodes_in, indexes_logs_nodes_out = OH.PT_num_P[0:3]
    ...
    antipodal_pairs = (jnp.linalg.norm((OH.X[indexes_logs_nodes_in] + OH.X[indexes_logs_nodes_out]), axis=-1) > 1e-3)[..., None]
    Logs = jax.vmap(Man.connec.log)(OH.X[indexes_logs_nodes_in]*antipodal_pairs,
    ...
    if jnp.isnan(jax.vmap(Man.connec.log)(OH.X[indexes_logs_nodes_in],
                                    OH.X[indexes_logs_nodes_out])).any():
        print("error in Logs found")
    # Logs = jax.ops.segment_sum(Logs, indexes_logs_edges_out, num_segments=len(OH.PT_num_P[6]))
    return Logs


def div_P_sym_weighted_squared(OH: OHGraphTupleReduced, H_f: jnp.array, Man: Manifold,
                               edge_normalize=False, deg_normalize=False, pairwise_summed=False):
    """
    ...
    indexes_PT_edges_in, indexes_PT_nodes_in, indexes_PT_nodes_out, indexes_P_edges_in_div, indexes_P_edges_segsum = OH.PT_num_P[3:]
    antipodal_pairs = (jnp.linalg.norm((OH.X[indexes_PT_nodes_out] + OH.X[indexes_PT_nodes_in]), axis=-1) > 1e-3)[
        ..., None]
    # antipodal_pairs = 1
    PTedH_f = jax.vmap(Man.connec.transp)(OH.X[indexes_PT_nodes_in]*antipodal_pairs, OH.X[indexes_PT_nodes_out]*
                                          antipodal_pairs, H_f[indexes_PT_edges_in])

    PTedH_f_summed = jax.ops.segment_sum(PTedH_f, indexes_P_edges_segsum, num_segments=len(OH.PT_num_P[6]))
    PTedH_f_summed_weighted = PTedH_f_summed * W[indexes_P_edges_in_div][..., None]
    div_unormalized = jax.ops.segment_sum(PTedH_f_summed_weighted, OH.PT_num_F[-1],
                                          num_segments=(len(OH.X)))
    div = div_unormalized / D_in[..., None]
    return div


def PLaplace(OH: OHGraphTupleReduced, Man: Manifold, edge_normalize=False, deg_normalize=False):
    """
    ...
    Laplace = -div_P_sym_weighted_squared(OH, gradsP, Man,
                                          edge_normalize=edge_normalize, deg_normalize=deg_normalize)
    return Laplace


def OH_Laplace(OH: OHGraphTupleReduced, Man: Manifold, laplace, edge_normalize=False, deg_normalize=False):
    """
...
def OH_Laplace_graph(OH: OHGraphTupleReduced, Man: Manifold,
                     edge_normalize=False, deg_normalize=False):
    """
...
def regularized_variation(data, laplace, OH: OHGraphTupleReduced, Man: Manifold, lambda_scalar=1,
                          edge_normalize=False, deg_normalize=False):
    """
    ...
    """
    laplace_val = laplace(OH, Man, edge_normalize=edge_normalize, deg_normalize=deg_normalize)
